...
export function SearchView({
  ...
  const folderName = useMemo(() => {
    ...
    const parts = currentFolderPath.replace(/\\/g, '/').split('/').filter(Boolean);
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pb-1 border-b text-xs">
        ...
        <div className="flex items-center gap-2 text-muted-foreground shrink-0 ml-auto">
          {isSearching ? (
            ...
            <span>
              <strong className="text-foreground">{total.toLocaleString()}</strong> results in{' '}
      ...
      {status?.isIndexing && (
        <div className="flex items-center justify-between px-3 py-1.5 rounded-md bg-amber-500/10 border border-amber-500/20 text-amber-900 dark:text-amber-200 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span>
              Search index is updating in the background. Search results may be incomplete until finished.
            </span>
          </div>
          <Button
            size="sm"
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        {isIndexEmpty && !query ? (
              ...
              <CardContent className="pt-8 pb-8 px-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Powered by Tantivy in Rust. Search your computer by file name and instant text
                    search inside files. Everything is indexed locally.
                  </p>
                </div>
                <div className="pt-2">
                  <Button onClick={() => onStartIndexing(false)} className="w-full gap-2">
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-8 text-muted-foreground">
            ...
            <h4 className="text-sm font-semibold text-foreground">Search files and text content</h4>
            <p className="text-xs max-w-sm mt-1">
              Type to search by name or file contents across {status?.docCount.toLocaleString() ?? 'all'} indexed items.
            </p>
            <div className="flex flex-wrap justify-center gap-1.5 mt-4 text-[11px] font-mono">
              ...
              <span className="px-2 py-0.5 rounded bg-muted">&quot;exact phrase&quot;</span>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-8 text-muted-foreground">
            ...
            <h4 className="text-sm font-medium text-foreground">No matches found</h4>
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 min-w-0">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="text-right text-[11px] text-muted-foreground hidden sm:block">
                          <div>{!hit.isDir ? formatBytes(hit.sizeBytes) : '—'}</div>
                    ...
                    {hit.snippet && (
                      <div className="mt-1.5 pl-7 text-[11px] text-muted-foreground bg-muted/30 rounded px-2 py-1 font-mono leading-relaxed line-clamp-2">
                        &hellip;{' '}
                        {renderHighlightedSnippet(hit.snippet.text, hit.snippet.highlights)}{' '}
                        &hellip;
                      </div>
                    )}
                  </div>
      ...
      <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-1 border-t">
        <div className="flex items-center gap-3">
          ...
          <span>
            <kbd className="rounded border bg-muted px-1 py-0.5 text-[10px]">Enter</kbd> Open
          </span>
          <span>
            <kbd className="rounded border bg-muted px-1 py-0.5 text-[10px]">Ctrl</kbd>+
            <kbd className="rounded border bg-muted px-1 py-0.5 text-[10px]">Enter</kbd> Explorer
          </span>
        </div>
        <div>
          {status && (
            <span>
              {status.docCount.toLocaleString()} items indexed
            </span>
          )}
        </div>
      </div>
 ...
 * Never uses dangerouslySetInnerHTML.
 */
function renderHighlightedSnippet(text: string, highlights: [number, number][]) {
  ...
  highlights.forEach(([start, end], idx) => {
    ...
    if (s > lastIndex) {
      elements.push(<span key={`sn-un-${idx}`}>{text.slice(lastIndex, s)}</span>);
    }
    if (e > s) {
      elements.push(
        <mark
